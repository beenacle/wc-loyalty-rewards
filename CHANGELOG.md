...
- Fixed: **"Prorate on partial refund" did nothing, and partial refunds never adjusted points.** Reversal only ran on a full status change to `refunded`/`cancelled`. Partial (and full) refunds now reconcile earned points when `refund_behavior = prorate` (via `woocommerce_order_refunded`); repeated partial refunds each claw back only the incremental amount. Proration is computed against the **same base points were earned on** (item subtotal net of coupons, plus tax/shipping only when those earning settings are enabled) rather than the gross order total, so a shipping- or tax-only refund no longer claws back item points. The reconciliation is two-directional: deleting or reducing a refund (`woocommerce_refund_deleted`) restores the over-reversed points and lifetime.
